...
func NewCipher(radix int, maxTLen int, key []byte, tweak []byte) (Cipher, error) {
	...
	// dimakogan: Changed this to ease on testing for now. Consider removing
	minLen = 2
...
func (c Cipher) EncryptWithTweak(X string, tweak []byte) (string, error) {
	...
	if (n < c.minLen) || (n > c.maxLen) {
		return ret, fmt.Errorf("message length %d is not within min and max bounds [%d,%d]", n, c.minLen, c.maxLen)
	}
